function Kich_Hoat_Xu_ly_JSON(Chuoi_Tham_so, Chuoi_Goi,
    Ham_Xu_ly_Sau_khi_Thuc_hien) {
    ...
    var Dia_chi = "http://localhost:3000/" + Chuoi_Tham_so
    Xu_ly_HTTP.open("POST", Dia_chi)
    Xu_ly_HTTP.send(Chuoi_Goi)

}

function Tao_Danh_sach_Tham_so(Chuoi, Ten_Tham_so) {
    var Danh_sach = []
    var M = Chuoi.slice(Chuoi.indexOf('?') + 1).split('&')
    M.forEach(Con => {
        let [Ten, Gia_tri] = Con.split('=')

        if (Ten == Ten_Tham_so)
            Danh_sach.push(Gia_tri)
    })

    return Danh_sach

}
// Xử lý trạng thái mua hàng
